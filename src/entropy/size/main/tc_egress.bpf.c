...
/* LRU HashMap for fixing ACKs on current packet from Ingress */
...
int tc_egress(struct __sk_buff *ctx) {
    ...
    /* Create SEQ key to translate egress SEQ to modified */
    ...
    /* Create ACK key to translate egress ACK to modified */
    ...
    if (pad_bytes >= 4) {  // Need minimum of 4 bytes available for encoding original length | I can shrink the packet and add the leftover to next packet too.
        ...
        __u32 tcp_payload_len_new = new_ip_len - ip_hl - tcp_hl;  // all host-byte order
        ...
        if (seq_egress_info) {  // seq_num exists in the map already; update it and fix current pkt seq_num
            ...
            if (highest_sent_data_len && !(tcp->seq < highest_sent_data_len)) {  // packet is *NOT* retransmit
                ...
                struct seq_egress_info nxt_seq_val = {
                    .seq_egress = seq_egress_info->seq_egress + bpf_htonl(tcp_payload_len_new),
                };
                bpf_map_update_elem(&seq_egress_fix, &seq_egress_key, &nxt_seq_val, BPF_EXIST);  // update
            }
        } else {  // First packet entry that needs padding
            struct seq_egress_info nxt_seq_val = {
                .seq_egress = tcp->seq + bpf_htonl(tcp_payload_len_new),  // network-byte order
            };
        ...
        __s64 tot_diff = 0;
        __u32 pad_bytes = pad_bytes;
        if (pad_bytes > 4) {
            __u32 max_load = pad_bytes - 4;  // leave last 4 bytes for original length
            __u32 key = 0;
            ...
            __u32 rand_off = 0;
            if (space)
                rand_off = bpf_get_prandom_u32() % (space + 1);
            if (bpf_skb_store_bytes(ctx, init_pkt_len, &rbb->bytes[rand_off], max_load, 0)) {
                return TC_ACT_SHOT;
            }
            tot_diff = csum_diff_u8_buf(&rbb->bytes[rand_off], max_load, 0);  // Random payload csum diff
            if (tot_diff < 0) {  // Returns negative code in case of failure
                return TC_ACT_SHOT;
            }
        }
        ...
        __be32 pad_bytes_be = bpf_htonl(pad_bytes);
        __s64 d2 = bpf_csum_diff(0, 0, (__be32 *)&pad_bytes_be, 4, (__u32)tot_diff);
        if (d2 < 0) return TC_ACT_SHOT;  // Returns negative code in case of failure
        tot_diff = d2;
        if (mdf_pkt_len < 4)
            return TC_ACT_SHOT;
        // Only need to copy pad_bytes which I can use to undo every change I made
        if (bpf_skb_store_bytes(ctx, (mdf_pkt_len - 4), &pad_bytes_be, 4, 0)) {
            return TC_ACT_SHOT;
        }
        ...
        if (bpf_l3_csum_replace(ctx, offsetof(struct iphdr, check), bpf_htons(ip_len), bpf_htons(new_ip_len), sizeof(u16))) {
        ...
        if (bpf_l4_csum_replace(ctx, offsetof(struct tcphdr, check), 0, (__u64)tot_diff, 0)) {
        ...
        if (bpf_l4_csum_replace(ctx, offsetof(struct tcphdr, check), curr_seq_num, translated_seq_num, 4)) {  // all network-byte order
            bpf_printk("Something went wrong with TCP->SEQ l4_csum_replace().\n");
            return TC_ACT_SHOT;
        }
        // L4-TCP PSEUDO-header (Pseudo-IP) checksum replace
        u16 old_tcp_len = ip_len - ip_hl;
        u16 new_tcp_len = new_ip_len - ip_hl;
        if (bpf_l4_csum_replace(ctx, offsetof(struct tcphdr, check), bpf_htons(old_tcp_len), bpf_htons(new_tcp_len), BPF_F_PSEUDO_HDR | 2)) {  // Change specifically for the Pseudo-header of TCP
